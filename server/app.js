...
app.get("/api/health", (_req, res) => res.json({ ok: true, orgAlias: config.orgAlias }));

app.post("/api/plan", async (req, res, next) => {
  try {
    const command = String(req.body?.command || "").trim();
    if (!command || command.length > 1000) throw new AppError("Command must be 1-1000 characters");
...
app.post("/api/runs/:id/approve", async (req, res, next) => {
  try {
    if (req.body?.confirmed !== true) throw new AppError("Explicit confirmation is required");
    res.json(await approve(req.params.id, req.body.approvalToken));
  } catch (error) { next(error); }
});

app.post("/api/runs/:id/rollback", async (req, res, next) => {
  try {
    if (req.body?.confirmed !== true) throw new AppError("Explicit rollback confirmation is required");
